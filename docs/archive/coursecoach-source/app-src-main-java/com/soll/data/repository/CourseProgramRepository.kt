...
class CourseProgramRepository @Inject constructor(
    ...
    fun observeStoredCoursePackage(packageFileName: String): Flow<CoursePackageModel?> = flow {
        emit(readStoredCoursePackage(packageFileName))
    ...
    fun readStoredCoursePackage(packageFileName: String): CoursePackageModel? {
        val file = File(packagesDir, packageFileName)
        if (!file.exists()) return null
        return runCatching { packageAdapter.fromJson(file.readText(Charsets.UTF_8)) }
            .onFailure { Timber.w(it, "Failed to parse stored course package %s", packageFileName) }
            .getOrNull()
    }
