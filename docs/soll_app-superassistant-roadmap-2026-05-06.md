...
- `insight/220db783f7ba`: the Habr/Yandex Tracker plugin-platform case study is validated through the public primary article and retained as a server-first extension security contract at `docs/knowledge/yandex-tracker-plugin-platform-soll-boundary.md`. Reuse its versioned manifest, least-privilege scopes/egress, brokered calls, server-held credentials, explicit scope-diff approval and audit/rollback patterns; treat LLM-generated code as an untrusted proposal. Do not add dynamic JavaScript, a WebView plugin host, plugin credentials or arbitrary model-generated API calls to Android. The first eligible pilot is one read-only disposable server extension over 3-5 frozen synthetic fixtures and must pass all seven documented promotion gates before any runtime integration.
- `openpencil-cbbbe156`: weak secondary signal about a local `.fig` design/prototyping tool. Keep it as a deferred desktop-local candidate for a concrete Soll app screen redesign; verify the upstream project, license, local/offline boundary, telemetry, `.fig` round-trip fidelity and IDE-agent permissions before running a bounded spike, while production UI remains reviewed Jetpack Compose code.
...
- `insight/af83aa26b648`: OpenAI Shell + Skills + Compaction pattern applied as a transport-neutral Soll agent context prototype. The profile declares capabilities explicitly, exposes only selected allowlisted shell tool IDs, publishes a lightweight skill index before loading selected instructions, and compacts old events into an inspectable objective/state/evidence checkpoint while retaining the recent tail. Android still does not execute raw shell commands or host an autonomous agent loop.
...
- `sciir-82k`: scientific image-reasoning dataset signal. The complete pinned 24.96 GB snapshot was integrity-checked and preliminarily audited, but label-pair defects, subject leakage, missing captions, no reproducible article-level split and no current Soll scientific-image workload keep adoption deferred; any future cleaned benchmark stays desktop/server-side while Android receives only validated result cards and approvals.
...
- `source-item/e1174ebfd950/64af1d1a2fd48283`: Anthropic agent-containment security review is complete. Apply the nine recommendations in `docs/security/anthropic-agent-containment-recommendations.md` to disposable desktop/server workers, capability-aware egress, scoped identities, untrusted-content handling and out-of-band audit; Android remains the approval and observability client. Production rollout stays gated on a synthetic trial passing all seven measurable promotion gates.
...
- `peft-v015-release` / `peft-v016-release` / `peft-v017-release` / `peft-v018-release`: one deferred Hugging Face PEFT adapter-routing cluster, not four Android or Today initiatives. Revisit it server-side only after Soll has a local PEFT/LoRA adapter registry, adapter-serving profile, base-vs-adapter eval suite, and rollback/provenance policy.
- `peft-v016-release`: Hugging Face parameter-efficient fine-tuning release signal. The monitored item's Meshtastic/offline-transport placement is a heuristic false positive; keep PEFT evaluation in a future desktop/server adapter-training scope and evolve the existing Android mesh/outbox contract only from real transport requirements.
...
- `emergence-world-0936930e`: long-horizon multi-agent evaluation signal, not a model leaderboard. Add its useful memory/tool-use/drift/early-warning questions as a soak/resilience layer on the existing desktop/server AgenticDataBench contour; keep the simulation runtime and provider credentials off Android, which remains a result and approval client.
...
- `meeting-hygiene-46bb283d`: weak methodological meeting-process signal. Keep it as a deferred desktop/server KB and meeting-note template candidate: agenda and meeting type in, decisions plus next steps/owners/deadlines out; Android should use existing Raw Note/Chat capture and Tasks review surfaces only after a concrete workflow, consent/privacy policy and human confirmation rules are defined.
- `yaff-zero-copy-protobuf-cefec43f`: C++/Protobuf zero-copy format signal. Upstream `yandex/yaff` v0.1.0 was built and benchmarked locally, but the current Android sync pipeline is Retrofit/Moshi JSON and has no Protobuf/C++ hot path; keep production adoption deferred and preserve the existing JSON contract.
- `source-item/0d75242b770a/e916238e51d2cfd0`: YTsaurus GPU-rendering source is deferred because the isolated worktree has no approved test cluster or Soll render workload. Preserve the desktop/server trial contract—`gpu_limit=1`, `pool_trees=[gpu]`, platform-provided driver overlay, modeset/DRM devices, Vulkan plus private Xorg, three bounded `vkcube` runs and one verified scene artifact—but do not add the graphics/cluster stack or credentials to Android.
- `source-item/0d75242b770a/97f971eb6ef1eec9`: the Yandex organization-graph signal is implemented for the existing Android task workspace as Room schema 24: scoped snapshots, direct bridge edges and a duplicate-free, reference-counted closure index. `SollRepository` caches live/compatibility graphs, skips unchanged rebuilds and uses the matching `open`/`all` snapshot after service failure; the existing task list's `Проект` filter calls the indexed descendant query without restoring the removed graph workspace. The 50,000-row mobile budget, migration test and adaptation limits are in `docs/knowledge/task-graph-reachability-index.md`.
- `3d-vlm-robotics`: embodied-robotics research signal, not an Android vision feature. Keep it deferred until Soll has a concrete manipulator and calibrated 3D inputs; then evaluate retrieval-augmented spatial planning in an isolated desktop/server simulation beside the existing `Гаджеты`/ESP contour, with Android limited to telemetry, evidence and explicit approval.
...
- `source-item/0d75242b770a/e916238e51d2cfd0` was audited against the Yandex GPU-rendering article and official YTsaurus GPU scheduling/statistics documentation. The article's successful path is a platform-and-workload integration, not an Android feature: allocate one GPU, use a cluster-compatible driver overlay, pass NVIDIA compute plus `/dev/nvidia-modeset` and matching `/dev/dri/card*`/`renderD*` devices, run the NVIDIA Vulkan ICD and Xorg DDX on a private virtual display, prove the same-GPU WSI/swapchain path with `vkcube`, then run the real engine and encode with a software codec when the selected GPU has no NVENC. This isolated worktree has no vendored raw source, `yt` client, container runtime, approved YTsaurus endpoint/pool/layer, render scene or Soll rendering baseline; the local NVIDIA probe also fails to initialize NVML. Therefore no test operation ran, the successful-trial acceptance criterion remains unmet, and the result is `audited_and_deferred_no_test_cluster` with 0 operations, 0 rendered frames and 0 measured runtime value. Do not fabricate a local/simulated success, add cluster/graphics dependencies to Android, or let Android submit render operations or hold cluster credentials. Reopen only when Soll owns a concrete non-sensitive render workload and a cluster owner approves a disposable non-production target. The desktop/server spike must start with an unprivileged `job_count=1`, `gpu_limit=1`, `pool_trees=[gpu]` admission operation, verify the exact driver/device/Vulkan/Xorg identity, pass three bounded `vkcube` runs, produce one fixed-input scene artifact with duration/frame count/size/SHA-256, attach YTsaurus GPU utilization/memory/wall-time plus cleanup evidence, and show a measurable gain over the named current baseline. Android may consume only server-produced status, metrics, artifacts and explicit approve/reject actions through existing Soll surfaces; it must not submit render operations or hold cluster credentials. The focused evidence contract is `Soll/outputs/source-processing/source-item-0d75242b770a-e916238e51d2cfd0-verification.md`.
...
- `openpencil-cbbbe156` is a deferred local design-tool candidate, not an implementation-ready Android feature. The task-provided record is only a short secondary claim that OpenPencil is a free local analogue of Claude Design for `.fig`-based design and prototyping with Claude Code/Cursor integration; it provides no upstream repository, version, license, platform requirements, model/dependency inventory, privacy boundary or verifiable output examples. Do not add OpenPencil, an IDE-agent bridge, generated code/assets or a `.fig` runtime to `soll_app` from this evidence. Its project alignment is a desktop-local, design-time spike only after a concrete redesign is approved: use one non-sensitive existing Compose screen and a disposable `.fig` copy, then compare import/export round-trip fidelity, layout and reusable-component quality, light/dark and size-state coverage, accessibility/semantics handoff, editability, latency/resource use and network/telemetry behavior against the current manual Compose workflow. Before execution, verify the canonical upstream project and release, license and asset/font rights, checksums/dependencies, offline claims and least-privilege Claude Code/Cursor access. Keep the `.fig` and screenshots as review artifacts rather than production truth; a human must approve the proposal, and any accepted result must be reimplemented and tested as ordinary Jetpack Compose code. Until those gates and an actual UI need exist, leave this item deferred with no dependency or product-surface change.
...
- `source-item/0d75242b770a/ce21bc830e1b94e0` (`ios-media-feed-yandex`) contributes a media-feed implementation contract at `docs/knowledge/media-feed-implementation-soll-app.md`, not approval to add autoplay video now. Ten practices from the Yandex Food iOS post are adapted to the current Android architecture: a typed server media contract, one flat virtualized list, presentation-level coordination, visible-media network priority, a lifecycle-owned bounded Media3 player pool, byte-bounded caches and short-video buffering, server-side content mixing, explicit poster/loading/failure states, network/battery/thermal degradation and production-safe observability. The current Chat and `Источники` surfaces remain text/image digest + article cards; `SollSourceItem` has no typed video descriptor, `RemoteLinkPreviewImage` is not a media cache, and `MusicPlaybackService` must stay independent. Reopen implementation only for a separately approved `Источники` scenario after budgets are declared for lifecycle correctness, first frame, playback, scroll/jank, main-thread stalls, memory/resources, network and energy/thermal behavior and a real-device 100-item long-feed smoke passes. The focused audit is `Soll/outputs/source-processing/source-item-0d75242b770a-ce21bc830e1b94e0-verification.md`.
- `source-item/0d75242b770a/97f971eb6ef1eec9` applies the Yandex bridge + reference-counted closure model to the existing `SollGateway.getTaskGraph` boundary. Room schema 24 stores `task_graph_snapshots`, `task_graph_nodes`, direct `task_graph_edges` and one `task_graph_reachability` row per `(scope, ancestor_id, descendant_id)` with `path_count`; the closure primary key makes forward descendant listing an indexed, non-recursive query without `DISTINCT`, while a reverse secondary index supports ancestor audits. Complete server graphs remain capped at 700 nodes and are rebuilt into separate `open`/`all` snapshots transactionally; a cyclic, malformed or over-50,000-row closure is rejected before the previous cache is replaced, and unchanged snapshots skip rebuilding. `SollRepository` writes successful live or existing 404-compatible task-board graphs and reads the last valid same-scope snapshot only after live paths fail. The existing task list adds a `Проект` chip filter whose production call to `getTaskGraphDescendants` resolves descendant task IDs; no separate graph workspace or server endpoint is introduced. Android deliberately does not copy PostgreSQL incremental add/remove functions, fat-edge background queues or the article's production latency claims because it receives complete snapshots and is not the authoritative writer. The durable SQL/schema contract is `docs/knowledge/task-graph-reachability-index.md`; focused evidence and value metrics are in `Soll/outputs/source-processing/source-item-0d75242b770a-97f971eb6ef1eec9-verification.md`.
...
- The Hugging Face PEFT v0.15.0-v0.18.0 records (`peft-v015-release`, `peft-v016-release`, `peft-v017-release`, `peft-v018-release`) repeat the same adapter-routing direction and therefore form a single deferred adapter-routing cluster. Source processing must preserve the four source links but must not create duplicate Today work from their overlapping recommendations. The cluster stays deferred until the desktop/server Soll runtime has all four prerequisites: a local PEFT/LoRA adapter registry with base-model compatibility and artifact provenance, an adapter-serving profile with load/unload, resource-limit and fallback behavior, a reproducible base-vs-adapter eval suite, and an approval-gated rollback/provenance policy. Only then may one bounded server-side routing spike compare quality, latency, memory and failure recovery before promotion. Do not add PEFT, LoRA weights, adapter selection or model loading to Android; `soll_app` may later consume only server-produced routing status, eval summaries and approval tasks. The canonical source-processing evidence remains `Soll/outputs/source-processing/peft-adapter-routing-cluster-20260713.md`.
- `peft-v016-release` does not provide a Meshtastic or offline note/task transport design. The official PEFT v0.16.0 release is about LoRA-FA, RandLoRA, C3A, quantization, adapter hotswapping and model/checkpoint compatibility; its `HF_HUB_OFFLINE` item tests model loading from a local Hugging Face cache. The monitored record explicitly fell back after `No providers available`, so its `mesh_resilience` classification and transport next step are a cross-domain false positive. Do not add PEFT, PyTorch, model training or a second outbox to Android. Soll already exposes mesh status/counters plus claim/ACK/retry and validates allowlisted `status`, `brief`, `note` and `task` payload types. If a real Meshtastic slice is later approved, base it on official protocol/device evidence and a compact versioned allowlisted envelope carrying stable ID, type, timestamp/TTL, priority and bounded body/reference; require deduplication and ACK only after durable local note/task insertion, preserve command rejection, and validate payload limits, loss, retry and idempotency on real hardware. PEFT itself belongs only in a future desktop/server adapter-training evaluation with a concrete workload and reproducible base-vs-adapter acceptance tests.
...
- `emergence-world-0936930e` is useful as a long-horizon evaluation methodology, not as the Habr headline's Claude/Grok leaderboard. The primary paper (`arXiv:2606.08367`) demonstrates a persistent multi-agent platform with separate memory systems, dynamically gated tools and full trajectory logging, but its reported cross-model outcomes are representative runs over fixed ten-agent populations, cost-tier model snapshots and imperfect social proxies; they must not rank or select Soll providers. Do not import Emergence World's 3D/social simulation, 120+ tool catalog, live feeds or multi-day execution into Android. Instead extend the existing AgenticDataBench desktop/server eval harness with a long-horizon soak/resilience layer for Soll's source-monitoring and KB/meta-coordinator workflow. The first spike should replay one deterministic, non-sensitive source-monitoring/KB scenario across multiple cycles with fixed roles, inputs, tool permissions, provider/model snapshots and seeds, plus bounded stale-memory, conflicting-source, tool-outage, denied-capability and prompt-injection events. Log full trajectories and compare task/artifact correctness, tool success/error/denied-call rates, memory writes/retrievals and stale or contradictory recall, retry/plan churn, policy drift, gate violations, recovery time, human interventions, latency and cost; compare early-window alerts with end-of-run outcomes and require at least three repeated runs per configuration before promotion. Keep runtime capability/approval gates, secret isolation and production-mutation blocks deterministic outside the model. Android must not execute the soak test or hold provider credentials; it may only consume server-produced run status, trajectory summaries, warnings and explicit approve/reject tasks through `Источники`/`Инсайты`/`Roadmap`/Tasks/Chat/`Ask Soll`.
...
- `meeting-hygiene-46bb283d` is a deferred process/UX candidate, not an implementation-ready meeting recorder or transcription feature. The task-provided record contributes a useful output checklist—meeting type and agenda, recorded decisions, next steps, responsible owners and deadlines—but gives no technical implementation, evaluation, privacy model or evidence for its headline claim. Preserve it as a desktop/server KB note and, only after a concrete Soll meeting workflow is selected, trial a meeting-note template that accepts user-provided notes or a consented transcript and produces a reviewable draft. Android alignment is through the existing Raw Note or Chat intake and Tasks review contract (`title`, `description`, `sourceRef`, `dueDate`); do not reinterpret the bounded `/record` Telegram command as a meeting recorder, add a transcription dependency, or create tasks/owners/deadlines automatically. Before promotion, define participant consent, storage/retention and deletion, speaker-attribution and transcript-accuracy checks, then require the user to confirm every decision and action item. Until those gates exist, leave the item deferred with no production code or UI change.
- `yaff-zero-copy-protobuf-cefec43f` was evaluated against `yandex/yaff` v0.1.0 at pinned revision `d6f74675374b587ce24112c284abd54a92090221`. Its official C++20/Conan benchmark suite built successfully and local five-repetition Release runs showed a real Protobuf-read advantage: YaFF Flat took `6.33 ns` versus Protobuf `32.64 ns` for 10 flat fields, `110.56 ns` versus `661.00 ns` for 100 fields, and `7.55 ns` versus `111 ns` for a hot hierarchy; on the 512 MiB cold hierarchy the gap narrowed to `238 ns` versus `307 ns`. Size is workload-sensitive: dense 50-field YaFF Flat was `419.00` bytes versus Protobuf `534.60`, while at 5% density it was `294.58` versus `31.77` bytes. These are upstream workloads, not the Soll app pipeline: `GET /api/v1/android/sync-status` currently flows through Retrofit/Moshi JSON, domain mapping and a Moshi JSON offline cache, and the app has no Protobuf, C++, NDK or CMake integration. Do not add YaFF or change the Android wire contract. The only viable future prototype is a trusted internal C++ read-only snapshot/filter sidecar that memory-maps checksummed, schema-versioned YaFF and still emits the existing JSON response; it requires a measured C++/Protobuf hotspot, real-schema benchmarks, at least 20% p95 CPU improvement, bounded size, compatibility/fuzz/fallback evidence and a stable lifetime owner. Direct Android/network decoding is rejected because YaFF currently has no Kotlin/Java binding and `ReadMessage` has no verifier for hostile buffers. The focused audit and value metric are in `Soll/outputs/source-processing/source-item-0d75242b770a-84a6a99a52dd1e61-verification.md`; keep the source deferred with no production dependency until those gates are met.
- `3d-vlm-robotics` is a deferred embodied-robotics research candidate, not an implementation-ready Android camera, VLM or robot-control feature. The public Habr source (`https://habr.com/ru/companies/ru_mts/articles/1035508/`) is a secondary overview: its useful claim that VLM spatial/physical reasoning needs explicit geometric grounding is supported by PhyBlock (`arXiv:2506.08708`), SpinBench (`arXiv:2509.25390`) and MV-RoboBench (`arXiv:2510.19400`), while Retrieval-Augmented Manipulation (RAM, `DOI:10.1126/scirobotics.aea2092`) is the closest manipulation result. However, the article's combined platform and example `query_grasp`, `query_stable_poses` and `check_collision` tools are an architectural synthesis, not a drop-in SDK or validated Soll contract. Do not add a 3D model database, depth/AR dependency, local VLM, manipulator UI or autonomous actuation to `soll_app`; the repository currently has camera/barcode capture and headless ESP gadgets, but no calibrated multi-view/depth input, robot kinematics, gripper or motion planner. If a concrete robot is selected later, place the first bounded spike in an isolated desktop/server robotics sandbox beside the existing `Гаджеты`/ESP contour: use one synthetic or simulated pick/place scenario with calibrated camera poses and known object geometry, store stable poses and grasp affordances, and expose deterministic tools for grasp lookup, reachability, collision and stability checks. The VLM may propose only a high-level plan; a simulator and conventional geometry/kinematics stack must verify every step, and no command may reach hardware during the spike. Record task success, spatial-relation and orientation errors, infeasible-plan rejection, collision/reachability failures, latency and compute cost against a deterministic non-VLM baseline. Promotion requires a named manipulator and user workflow, primary paper/code/license verification, reproducible simulation gains, sensor/calibration and safe-state design, explicit capability/approval gates, emergency stop and real-device QA. Android should reuse existing `Гаджеты`, `Источники`, `Инсайты`, Roadmap and Tasks surfaces only to show server-produced scene/plan evidence, infeasibility reasons, telemetry and approve/reject actions; it must never send VLM-produced coordinates directly to actuators. Until those gates exist, leave the candidate deferred with no production code, dependency or UI change.
...
- `source-item/94b02ac6da81/f4c9532907757955` produced a bounded research prototype that adapts object detection's "what/where" contract to Soll's "what/when" event sequence. `AssistantEvent.type/source/createdAt` becomes an event class plus a predicted temporal window; recurring classes use a robust median period and deviation-derived tolerance, detector-style confidence and temporal IoU matching, while one-to-one assignment prevents duplicate credit and temporal NMS suppresses overlapping predictions. A deterministic synthetic Soll-shaped holdout reached F1 `0.9412` (8 TP, 0 FP, 1 FN) versus `0.6154` (4 TP, 0 FP, 5 FN) for a single-class frequency baseline, with mean matched timing error `0.125` day. This proves the adaptation and evaluator, not real-world predictive value: the raw source and a representative consented AssistantEvent history are absent from this isolated worktree, the fixture has deliberately regular series, and no calibration, drift, class-imbalance or operational-utility result exists yet. The implementation remains research-only and is not wired into notifications, task mutation, UI or background work. Re-evaluate offline on a redacted chronological export with train/validation/test splits, rolling-origin backtests, precision/recall/F1, timing MAE, calibration, false-alert budget and naive seasonal/last-interval baselines; promote only if repeated unseen-data gains translate to a named, reviewable user action without autonomous mutation.
- `source-item/854a3820824d/17b09ac75ba609e6` contributes a detailed launch/reliability checklist, not a request to turn `soll_app` into a trading bot. The current Android project has no exchange, order, position, OHLC, PnL, commission, slippage or leverage domain, and its legacy Telegram `BotService` is archived; trading-specific adoption therefore stays rejected. The durable adaptation lives in `docs/knowledge/trading-bot-launch-checklist-soll-app.md`: path-aware exits become authoritative effect receipts, look-ahead prevention becomes an explicit as-of boundary, fees/slippage become net latency/cost/battery/retry budgets, small samples become `N/A` plus chronological promotion gates, exchange acknowledgement becomes requested/accepted/executed/persisted state, portfolio exposure becomes aggregate resource budgets, and cron data becomes an injected replay clock with frozen fixtures. The audit identifies three concrete but separately scoped durability gaps: stale `RUNNING` sync/tool rows lack generic reconciliation, queued upload files are not atomically staged and validated, and ambiguous retryable writes lack an explicit end-to-end idempotency key. This source has measurable knowledge/audit value but zero measured trading runtime value; preserve the gates and open narrowly approved reliability tasks rather than adding trading code.
- `source-item/94b02ac6da81/43712e77076a40a8` adds the Habr Sber OLAP/semantic-layer signal to the BI knowledge base at `docs/knowledge/bi-semantic-layer-financial-reporting.md`. The reusable pattern moves ingestion and governed facts/dimensions behind one semantic contract so web/Excel/LibreOffice clients do not own divergent business formulas; the note defines metric passports, access/lineage controls, a current-repository evidence matrix and 6 adoption gates for one report-owning pilot. The isolated `soll_app` checkout contains 0 current financial reports: its API, Room model and dependencies cover tasks/chat/sources/devices/media, `FilesHandler` only labels Excel extensions and the existing Device QA Markdown is not financial reporting. Therefore no OLAP/BI/finance/Excel runtime dependency or Android UI was added, and the source has 0 measured report-automation value here. Reopen implementation only in the separately approved repository that owns a real financial report, its data and metric owner, with a manual-time/error baseline, reconciliation, access tests, two accepted reporting cycles and rollback.
- `task:chat:962471563b17ded7b120` adds the deep Bonsai 27B feasibility analysis at `docs/knowledge/bonsai-27b-on-device-android-feasibility.md`. The 1-bit 3.9 GB footprint makes a bounded opt-in text-only `arm64-v8a` compatibility spike plausible through upstream `llama.cpp`, but Android production integration is not approved: PrismML's 11 tok/s proof is iPhone/MLX-only, the first 4K text peak is already about 5.2 GB before the rest of Soll/Android, and the 89.5% aggregate hides agent/tool-calling degradation from 80.0 to 66.03. The article's 5.9 GB ternary number is the ideal representation; current deployed Q2_0 is about 7.2 GB. Keep the existing server route as default, do not bundle weights or execute model tool calls directly, and start only after naming a target device, pinning model/runtime revisions and freezing prompts. A separate text-only spike must use 4K context without vision/drafter and pass all 8 promotion gates for compatibility, PSS/LMK, speed, thermal/battery, Soll-specific quality, tool safety, privacy/offline behavior and delivery/rollback. Actual Android Bonsai runs and measured runtime value remain 0; evidence is `Soll/outputs/source-processing/task-chat-962471563b17ded7b120-verification.md`.
