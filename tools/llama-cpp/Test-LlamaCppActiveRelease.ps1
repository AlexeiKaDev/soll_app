...
if ([int] $config.release.tag.TrimStart("b") -lt [int] $config.policy.minimumSafeRelease) {
    throw "Active llama.cpp release does not satisfy the configured security baseline"
}
if (-not $config.policy.verifySha256 -or -not $config.policy.requireModelAllowlist) {
    throw "Active llama.cpp policy must verify release hashes and require the model allowlist"
}
...
$results = foreach ($targetId in $Target) {
    ...
    if ($targetId -eq "windows-x64-cpu" -and
        ...
        if ($version -notmatch $targetConfig.versionPattern) {
    ...
    [pscustomobject]@{
        ...
        securityBaseline = $config.release.securityBaseline
